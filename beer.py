...
# class Beer_correlated(BeerData):
#     def __init__(self,data_dir, aspect, mode, word2idx, balance=False, max_length=256, neg_thres=0.4, pos_thres=0.6,
#                  stem='reviews.260k.{}.txt.gz'):
#         super(Beer_correlated, self).__init__(data_dir, aspect, mode, word2idx, balance, max_length, neg_thres, pos_thres,
#                  stem)
#         self.input_file = os.path.join(data_dir, '/reviews.260k.{}.txt.gz'.format(mode))
#     def _create_examples(self, aspect, balance=False):
#         examples = []
#         with gzip.open(self.input_file, "rt") as f:
#             lines = f.readlines()
...
class BeerData1(Dataset):
    ...
    def _convert_single_text(self, text, max_length, word2idx):
        ...
        try:
            i1 = input_ids.index(word2idx['.'])
        except:
            i1 = 256
        try:
            i2 = input_ids.index(word2idx['?'])
        except:
            i2 = 256
        try:
            i3 = input_ids.index(word2idx['!'])
        except:
            i3 = 256
